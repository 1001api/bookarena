...
// MaskEmailStart masks an email address by showing only the first 3 characters
// of the local-part, replacing the rest with asterisks, and keeping the domain intact.
// Example: input "john.doe@example.com" -> output "joh****@example.com"
func MaskEmail(email string) string {
	parts := strings.Split(email, "@")
	if len(parts) != 2 {
		// return unchanged if not a valid email format
		return email
	}

	localPart, domain := parts[0], parts[1]

	// if local-part is 3 or fewer characters, return unchanged
	if len(localPart) <= 3 {
		return email
	}

	// keep first 3 characters, mask the rest
	firstThree := localPart[:3]
	maskedLength := len(localPart) - 3
